...
IpaReader.prototype.parsePlist = function(){
  var manifest = {}
    ...
    log.info("初始化destdir = " + destdir)

    var dirs = fs.readdirSync(destdir)
    log.info("初始化dirs = " + dirs)

    destdir = util.format("%s/%s",destdir,dirs[0])
    log.info("处理后dirs = " + destdir)

    var destfile = destdir+'/Info.plist'
    log.info("destfile = " + destfile)

    //把info.list的xml格式转换成binary格式，不然解析时没有bplist
    var cmd = "plutil -convert binary1 " + destfile
    execSync(cmd,{});
    ...
    return new Promise((resolve,reject)=>{
      bplist.parseBuffer(content,function(err,result){
        ...
        log.info("package："+ result[0].CFBundleIdentifier)
        log.info("versionCode："+ result[0].CFBundleInfoDictionaryVersion)
        log.info("versionName："+ result[0].CFBundleShortVersionString)
...
IpaReader.prototype.UnzipIpa = function(){
    this.cachedir = os.tmpdir()+'/ipa'
    log.info("cachedir = " + this.cachedir)

    if(fs.existsSync(this.cachedir)){
      var cmd = 'rm -rf '+this.cachedir
      // console.log(cmd)
      execSync(cmd,{});
      log.info("删除旧的.ipa文件，执行命令为：" + cmd)
    }

    fs.mkdirSync(this.cachedir)
    log.info("创建文件夹，路径为" + this.cachedir)
   
    return new Promise((resolve,reject)=>{
      fs.createReadStream(this.file )
      .pipe(unzip.Extract({path:this.cachedir})).on('close',()=>{
        log.info("解压文件（" + this.file.toString() +"），读取文件内容信息")
        return resolve()
      })
    })
}
