...
function getUsersList(req, res) {
  dbapi.getUsersList()
    ...
    .catch(function (err) {
      log.error('Failed to load user list: ', err.stack)
      res.status(500).json({
        success: false
      })
    })
    ...
    .then(function (user) {
      if (user) {
        return res.status(202).json({
          success: false
          , description: '邮箱已存在，请重新输入'
        })
      }

      dbapi.insertUser(name, email, password, ip)
        .then(function () {
          return res.json({success: true, status: 'SUCCESS'})
        })
    })
...
function deleteUserByEmail(req, res) {
  ...
  dbapi.deleteUserByEmail(email)
    ...
    .catch(function (err) {
      log.error('Failed to delete user by  "%s": ', email, err.stack)
      res.status(500).json({
        success: false
      })
    })
