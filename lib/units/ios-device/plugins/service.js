  ...
  .define(function(options, wda, router, push,storage) {
    ...
    plugin.rout = function(){
        router
          ...
          .on(wire.QueryAppListMessage, function(channel, message) {
            var appList
            var result = plugin.queryAppList(options.serial)
              // .then(function(result) {
              appList = result
              // log.info("Weget:" + appList)
              var reply = wireutil.reply(options.serial)
              push.send([
                channel
                , reply.okay('success',{
          ...
          .on(wire.LaunchAppMessage, function(channel, message) {
            var reply = wireutil.reply(options.serial)
            plugin.launchDashApp(message.bundleId)
            push.send([
              channel
              , reply.okay('success',{
              })
          ...
          .on(wire.TerminateAppMessage, function(channel, message) {
            var reply = wireutil.reply(options.serial)
            plugin.stopDashApp(message.bundleId)
            push.send([
              channel
              , reply.okay('success',{
              })
          ...
          .on(wire.UninstallAppMessage, function(channel, message) {
            var reply = wireutil.reply(options.serial)
            plugin.uninstallDashApp(message.bundleId, options.serial).then(function(result) {
              push.send([
                channel
                , reply.okay('success',{
                })
          ...
          .on(wire.WeakNetSetMessage, function(channel, message) {
            var reply = wireutil.reply(options.serial)
            plugin.WeakNetGetTokenAndAddress().then(function(resp) {
              push.send([
                channel
                , reply.okay('success',{
                  resp: resp.value
