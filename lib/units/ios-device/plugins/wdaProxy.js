...
.define(function(options,wda){
  ...
  plugin.startWda = function(){
    ...
    //      "--uninstall","com.apple.test.WebDriverAgentRunner-Runner"])
    //uninstall.start()
    var params = ['build-for-testing', 'test-without-building','-project','/usr/local/lib/node_modules/WebDriverAgent/WebDriverAgent.xcodeproj'/*path.join(wdaPath,'WebDriverAgent.xcodeproj')*/
                  ,'-scheme','WebDriverAgentRunner','-destination','id='+options.serial+',platform=iOS'+platform
                  ,'-configuration','Debug','IPHONEOS_DEPLOYMENT_TARGET=13.2']
