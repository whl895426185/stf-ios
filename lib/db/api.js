...
dbapi.setDeviceSupportAutomation = function(serial, supportAutomation) {
  return db.run(r.table('devices').get(serial).update({
    supportAutomation: supportAutomation
  }))
}
...
dbapi.getUsersList = function(){
    return db.run(r.table('users').orderBy(r.desc('createdAt')))
}

dbapi.deleteUserByEmail = function(email){
  return db.run(r.table('users').get(email).delete())
}
dbapi.getUserByEmail = function(email){
  return db.run(r.table('users').get(email))
}
dbapi.insertUser = function(name, email, password, ip) {
  return db.run(r.table('users').insert({
    name: name,
    email: email,
    password: password,
